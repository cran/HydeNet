...
plot_nondeterm_only <- function(network){
  non_determ_nodes <- 
    network[["nodes"]][!vapply(X = network$nodeType,
                               FUN = function(x) x == "determ",
                               FUN.VALUE = logical(1))]
  non_determ_nodes <- non_determ_nodes[!is.na(non_determ_nodes)]
  
  form_parts <- 
    vapply(X = non_determ_nodes, 
           FUN = non_determ_parents_form,
...
non_determ_parents_form <- function(node, network)
{
  ...
  while(any(determ_parent(parent_types)))
  {
    ...
    super_parents <- do.call("c", lapply(X = names(parent_types[have_determ]),
                                         FUN = parent_type,
                                         network))
    
    parent_types <- c(parent_types[!names(parent_types) %in% have_determ], 
                      super_parents)
  }
  
  form <- paste0(node, " | ", 
                 paste0(unique(names(parent_types)), 
                        collapse = " * "))
  gsub(pattern = " [|] $", 
       replacement = "", 
       x = form)
...
{
  vapply(X = parent_types, 
         FUN = function(pt) "determ" %in% pt, 
         FUN.VALUE = logical(1))
}
